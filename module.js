...
(function (window, document) {
  ...
  function bindEvents() {
    ...
    backButton && backButton.addEventListener('click', redirectToDashboard);
    logoutButton && logoutButton.addEventListener('click', handleLogout);

    refreshButton && refreshButton.addEventListener('click', async () => {
      setButtonLoading(refreshButton, true, 'กำลังรีเฟรช...');
    ...
    calendarButton && calendarButton.addEventListener('click', openCalendar);
    ...
    statusFilter && statusFilter.addEventListener('change', () => {
      state.statusFilter = String(statusFilter.value || 'ALL').toUpperCase();
      applyFiltersAndRender();
    });
  ...
  async function loadRecords(options) {
    ...
    try {
      ...
      updateServerOffset(result.generatedAt);
      ...
      setText(
        ...
        'อัปเดตล่าสุด ' +
          (
            result.generatedAt ||
            formatBangkokDateTime(getCurrentServerDate())
          )
      );
      ...
      if (!config.silentError) {
        await showApiError(error, 'โหลดข้อมูลไม่สำเร็จ');
      }
